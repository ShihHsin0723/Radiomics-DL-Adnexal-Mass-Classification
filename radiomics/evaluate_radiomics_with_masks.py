    ...
    def __init__(self, image_dir, radiomics_dir, feature_map1, feature_map2, transform=None):
        ...
        for label, cls in enumerate(self.classes):
            ...
            for img_name in os.listdir(class_path):
                if "_image.jpg" not in img_name:  # Ensure only processing images
                    continue

                img_path = os.path.join(class_path, img_name)
                mask_path = img_path.replace("_image.jpg", "_mask.jpg")  # Get corresponding mask path
                ...
                if os.path.exists(path1) and os.path.exists(path2) and os.path.exists(mask_path):
                    rfm1 = np.load(path1)
                    rfm2 = np.load(path2)
                    ...
                    self.image_paths.append((img_path, mask_path))
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for val_inputs, val_labels in val_loader:
                ...
                y_probs = torch.softmax(val_outputs, dim=1)[:, 1]
                val_predicted = (y_probs >= THRESHOLD).long()
...
def evaluate_model(model, test_loader):
    ...
    best_thresh = get_best_threshold_by_youden_index(all_labels, all_probs)
    print(f"Optimal threshold (Youden Index): {best_thresh:.4f}")
    
    return test_loss, test_acc, sensitivity, specificity, f1, auc

def get_best_threshold_by_youden_index(y_true, y_scores):
...
if __name__ == "__main__":
    ...
    TEST_IMAGE_DIRS = [
        "/home/phoebe0723/rop2/segmented_data/test",
    ...
    # torch.save(model.state_dict(), "efficientnet_radiomics_with_mask.pth")
    # print("Trained model saved to efficientnet_radiomics_with_mask.pth")

    # Load trained model
    model = EfficientNetClassifier(num_classes=2)
    model.load_state_dict(torch.load("efficientnet_radiomics_with_mask.pth", map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu")))
    model.eval()
    print("Loaded trained model from efficientnet_radiomics_with_mask.pth")
    ...
    for level in range(levels):
        ...
        csv_file = f"./noisy_results_new_split/{dataset_name}_with_masks.csv"
        ...
        test_loader = DataLoader(balanced_test_dataset, batch_size=BATCH_SIZE, shuffle=True)
